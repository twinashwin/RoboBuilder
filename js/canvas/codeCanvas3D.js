...
const CodeCanvas3D = (() => {
  ...
  if (typeof THREE === 'undefined') {
    console.warn('[CodeCanvas3D] THREE not loaded — module disabled.');
    return null;
  }

  // Coordinate mapping — must match BuildCanvas3D.getRobotConfig SCALE/OFFSET.
  const SCALE      = 40;
  const OFFSET_X   = 200;
  const OFFSET_Y   = 150;
  ...
  function init(containerEl) {
    container = typeof containerEl === 'string'
      ...
      : containerEl;
    if (!container) {
      console.error('[CodeCanvas3D] Container not found');
      return;
    }
    ...
    renderer.domElement.id = 'code-canvas-3d';
  ...
  function rebuildBarriers() {
    ...
    const segments = [
      [(minX + maxX) / 2, minZ - wt / 2, (maxX - minX) + wt * 2, wt],
      [(minX + maxX) / 2, maxZ + wt / 2, (maxX - minX) + wt * 2, wt],
      [minX - wt / 2, (minZ + maxZ) / 2, wt, (maxZ - minZ)],
      [maxX + wt / 2, (minZ + maxZ) / 2, wt, (maxZ - minZ)],
    ];
  ...
  function rebuildRobot() {
    ...
    // Centroid in build coords (so the robot pivots about its center, matching
    // the 2D sim canvas).
    let sumX = 0, sumY = 0;
    const partInfo = robotConfig.parts.map(p => {
    ...
    const comX = sumX / robotConfig.parts.length;
    const comY = sumY / robotConfig.parts.length;
...
if (typeof window !== 'undefined') window.CodeCanvas3D = CodeCanvas3D;
